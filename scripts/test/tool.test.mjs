...
test('bookmarklet: chỉ đọc, bắt socket rồi trả getter, hiển thị trade/wild, tắt sạch', async t => {
  ...
  assert.equal(root.querySelectorAll('.tcard').length ? root.querySelector('.tcard').querySelectorAll('.tier').length : 0, 2, 'trade: hạng cho cả con đưa và con nhận');
  assert.ok(root.querySelector('.tcard.is-ok button.act.do-trade'), 'slot có sẵn lính phải có nút Trade');
  assert.doesNotMatch(text(), /Vàng/, 'không lặp lại chỉ số game đã hiện');
  assert.ok(root.querySelector('.tcard.is-ok'), 'slot trade được phải được đánh dấu');
  assert.equal(root.querySelectorAll('.info').length, 0, 'không còn nút i — bấm ảnh pet là mở thông tin');
  ...
  const strip = root.querySelector('.tile .strip');
...
test('bookmarklet: nút Bắt/Tiến hóa/Trade gọi đúng hàm game, chỉ khi người bấm, 1 cú = 1 lệnh', async t => {
  ...
  await tick(1200);
  const root = log.roots[0];
  const KIND = { Trade: 'trade', 'Bắt': 'catch' };
  const btn = text => root.querySelector(`button.act.do-${KIND[text]}`);
  ...
  await tick(850);
  tab('Đội');
  const up = root.querySelector('button.act.do-evolve');
  trustedClick(w, up);
  ...
  await tick(850);
  trustedClick(w, root.querySelector('.tile.pick .pic'));
  assert.equal(calls.at(-1)[0], 'selectEntity');
  assert.ok(!root.querySelector('.drawer').hidden, 'bấm ảnh pet → vừa chọn trong game vừa mở thông tin');
...
test('bookmarklet: nhãn + đích của nút lấy từ catalog GAME, overlay giả mạo không đổi được; không thao tác nhà người khác', async t => {
  ...
  const ups = [...root.querySelectorAll('button.act.do-evolve')];
...
test('bookmarklet: bản m.cutd.site dán giữa trận — nút khoá vài giây rồi tự Móc (mở lại trận trong khung)', async t => {
  ...
  trustedClick(w, root.querySelector('.tcard .pic.pick'));
...
test('bookmarklet: bản web — dán từ sảnh → móc session/interaction lúc game tạo, gọi thẳng hàm game, gỡ bẫy sạch', async t => {
  ...
  const trustedDown = el => { const ev = new w.MouseEvent('pointerdown', { bubbles: true, composed: true }); jsdomUtils.implForWrapper(ev).isTrusted = true; jsdomUtils.implForWrapper(el)._dispatch(jsdomUtils.implForWrapper(ev)); };
  const press = () => { trustedDown(btn('Bắt')); trustedClick(w, btn('Bắt')); };
  await tick(850);
  let n = w.__calls.length;
  [...root.querySelectorAll('.tab')].find(b => b.textContent.startsWith('Wild')).click();
  press();
  ...
  await tick(400);
  press();
  assert.equal(w.__calls.length, n + 1, 'nhìn lại rồi bấm → ăn');
...
test('bookmarklet: Xếp đội — mỗi lần bấm dời đúng 1 con lệch hàng, con đúng hàng không đụng, chờ game xác nhận, chỉ lúc chuẩn bị', async t => {
  ...
  assert.ok(ranged, 'có con hồi máu / hào quang để thử hàng cuối');
  w.eval(`
    window.__moves = [];
  ...
  assert.ok(drawer().hidden, 'chưa bấm ảnh pet → không có khung thông tin');
  root.querySelector('.tile .pic').click();
  assert.ok(!drawer().hidden, 'bấm ảnh pet → mở khung thông tin');
  assert.ok(root.querySelector('.tile.sel'), 'ô đang xem được đánh dấu');
  assert.ok(drawer().querySelector('.dh b').textContent.length > 0, 'có tên pet');
  assert.ok(drawer().querySelectorAll('.stats span').length === 4, '4 chỉ số dạng ký hiệu');
  assert.equal(drawer().querySelectorAll('a, button').length, 0, 'không nút, không link');
  drawer().dispatchEvent(new w.MouseEvent('pointerdown', { bubbles: true, composed: true }));
  assert.ok(!drawer().hidden, 'bấm trong khung → vẫn mở');
  w.document.body.dispatchEvent(new w.MouseEvent('pointerdown', { bubbles: true, composed: true }));
  assert.ok(drawer().hidden, 'bấm ra ngoài → khung biến mất, chỉ còn tool');
  root.querySelector('.tile .pic').click();
  root.querySelector('.tabs').dispatchEvent(new w.MouseEvent('pointerdown', { bubbles: true, composed: true }));
  assert.ok(drawer().hidden, 'bấm chỗ khác trong tool → cũng đóng');
  const names = () => [...root.querySelectorAll('.tile .tn')].map(r => r.textContent);
  const last = names().at(-1);
  assert.ok(names().length >= 2 && names()[0] !== last);
  [...root.querySelectorAll('.tile')].at(-1).querySelector('.star').click();
  assert.equal(names()[0], last, '★ wishlist đưa dòng đó lên đầu tab Đội');
  assert.ok(root.querySelector('.tile .star.on'));
  assert.match(root.querySelector('.sect').textContent, /★/, 'wishlist có nhóm riêng trên đầu');
  root.querySelector('.tile .star.on').click();
...
test('bookmarklet: nhãn vai trò (ATK/TANK/BUFF…) + lọc theo vai trò ở tab Wild', async t => {
  ...
  const primary = { atk: 'ATK', tank: 'TANK' }[overlay.u[scenario.a].kt?.[0]];
  assert.ok(primary, 'có vai chính');
  assert.equal(tiles()[0].style.getPropertyValue('--ro'), primary === 'ATK' ? '#ff8f8f' : '#7fd4ff', 'vạch màu trên ô = vai trò');
